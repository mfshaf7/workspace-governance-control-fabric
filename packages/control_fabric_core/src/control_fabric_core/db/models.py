...
class WorkspaceIntakeEvaluationRecord(TimestampMixin, Base):
    """Immutable intake evaluation; never a canonical classification record."""

    __tablename__ = "workspace_intake_evaluations"
...
class WorkspaceInventoryEvaluationRecord(TimestampMixin, Base):
    """Immutable active-inventory readiness artifact and caller binding."""

    __tablename__ = "workspace_inventory_evaluations"
...
class PrototypeLandingReadinessRecord(TimestampMixin, Base):
    """Immutable Prototype Landing readiness artifact and caller binding."""

    __tablename__ = "prototype_landing_readiness"

    evaluation_id: Mapped[str] = mapped_column(String(192), primary_key=True)
    evaluation_digest: Mapped[str] = mapped_column(String(71), nullable=False)
    readiness_digest: Mapped[str] = mapped_column(String(71), unique=True, nullable=False)
    actor: Mapped[str] = mapped_column(String(256), nullable=False)
    authority_revision: Mapped[str] = mapped_column(String(40), nullable=False)
    contract_digest: Mapped[str] = mapped_column(String(71), nullable=False)
    implementation_ref: Mapped[str] = mapped_column(String(40), nullable=False)
...
class PrototypeMaturityReadinessRecord(TimestampMixin, Base):
    """Immutable Prototype maturity readiness artifact and caller binding."""

    __tablename__ = "prototype_maturity_readiness"
...
class PrototypeClosureReadinessRecord(TimestampMixin, Base):
    """Immutable Prototype Closure readiness artifact and caller binding."""

    __tablename__ = "prototype_closure_readiness"
