...
- `apps/api`: future FastAPI service surface after platform and security gates.
...
The scaffold deliberately keeps API, database, and worker behavior out of the
first source slice. Those capabilities are separate ART children so their
contracts, tests, and review evidence stay bounded.
